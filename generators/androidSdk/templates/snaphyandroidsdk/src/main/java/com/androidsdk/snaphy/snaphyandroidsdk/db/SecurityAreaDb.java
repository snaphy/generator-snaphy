...
public class SecurityAreaDb{
  ...
  public SecurityAreaDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<SecurityArea> getAll__lazy(final int limit, final int skip, ObjectCallback<SecurityArea> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final SecurityAreaRepository repo = restAdapter.createRepository(SecurityAreaRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<SecurityArea>() {
            @Override
            public SecurityArea create(Cursor cursor, int index) {
                SecurityArea data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (SecurityArea)repo.createObject(hashMap);
                return data;
            }
    ...
    public SecurityArea get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("SecurityArea", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
    ...
    public void update__db(final String id,   final SecurityArea _modelData) {
      ...
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`SecurityArea`", values, "id = ?", new String[] { id });
    }
