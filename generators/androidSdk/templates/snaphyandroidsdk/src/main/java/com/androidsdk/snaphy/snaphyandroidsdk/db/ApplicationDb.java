...
public class ApplicationDb{
  ...
  public ApplicationDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<Application> getAll__lazy(final int limit, final int skip, ObjectCallback<Application> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final ApplicationRepository repo = restAdapter.createRepository(ApplicationRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Application>() {
            @Override
            public Application create(Cursor cursor, int index) {
                Application data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Application)repo.createObject(hashMap);
                return data;
            }
    ...
    public Application get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Application", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
