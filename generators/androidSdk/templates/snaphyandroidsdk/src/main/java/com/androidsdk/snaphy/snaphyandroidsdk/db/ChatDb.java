...
public class ChatDb{
  ...
  public ChatDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
    ...
    public void insert__db (final String id, final Chat _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      // Inserting Row
    ...
    public ContentValues getContentValues(Chat _modelData){
    ...
    private HashMap<String, Object> parseCursor(Cursor cursor ){
      HashMap<String, Object> hashMap = new HashMap<>();
                        ...
                        if(cursor.getString(5) != null){
                          messageData = cursor.getString(5);
                          if(messageData != null){
     ...
     * @return
     */
    public LazyList<Chat> getAll__lazy(final int limit, final int skip, ObjectCallback<Chat> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final ChatRepository repo = restAdapter.createRepository(ChatRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Chat>() {
            @Override
            public Chat create(Cursor cursor, int index) {
                Chat data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Chat)repo.createObject(hashMap);
                return data;
            }
            ...
            public String getUpdateQuery() {
                ...
                if(orderBy != null){
                    ...
                    if(limit != 0){
                        // Select All Query
                        selectQuery = "SELECT  * FROM Chat " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
                    }else{
                        selectQuery = "SELECT  * FROM Chat " + whereQuery  + " LIMIT -1 OFFSET " + skip;
                    }
                }
    ...
    public Chat get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Chat", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
    ...
    // Getting All Data where and sort column according to date wise..
    public DataList<Chat>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<Chat> modelList = new DataList<Chat>();
    ...
    // Getting All Data where
    public DataList<Chat>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM `Chat` " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                countQuery = "SELECT  * FROM `Chat` " + whereQuery + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final Chat _modelData ){
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      String where = getWhere(whereKeyValue);
      db.update("`Chat`", values, where, null);
    }
    ...
    public void update__db(final String id,   final Chat _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`Chat`", values, "id = ?", new String[] { id });
    }
