...
public class IpdBedDb{
  ...
  public IpdBedDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
    ...
    public IpdBed get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("IpdBed", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
    ...
    public void update__db(final String id,   final IpdBed _modelData) {
      ...
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`IpdBed`", values, "id = ?", new String[] { id });
    }
