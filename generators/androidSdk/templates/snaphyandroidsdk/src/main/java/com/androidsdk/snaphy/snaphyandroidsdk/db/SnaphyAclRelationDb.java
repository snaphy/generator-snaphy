...
public class SnaphyAclRelationDb{
  ...
  public SnaphyAclRelationDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<SnaphyAclRelation> getAll__lazy(final int limit, final int skip, ObjectCallback<SnaphyAclRelation> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final SnaphyAclRelationRepository repo = restAdapter.createRepository(SnaphyAclRelationRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<SnaphyAclRelation>() {
            @Override
            public SnaphyAclRelation create(Cursor cursor, int index) {
                SnaphyAclRelation data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (SnaphyAclRelation)repo.createObject(hashMap);
                return data;
            }
    ...
    public SnaphyAclRelation get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("SnaphyAclRelation", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
