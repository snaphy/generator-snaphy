...
public class AmazonImageDb{
  ...
  public AmazonImageDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<AmazonImage> getAll__lazy(final int limit, final int skip, ObjectCallback<AmazonImage> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final AmazonImageRepository repo = restAdapter.createRepository(AmazonImageRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<AmazonImage>() {
            @Override
            public AmazonImage create(Cursor cursor, int index) {
                AmazonImage data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (AmazonImage)repo.createObject(hashMap);
                return data;
            }
    ...
    public AmazonImage get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("AmazonImage", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
