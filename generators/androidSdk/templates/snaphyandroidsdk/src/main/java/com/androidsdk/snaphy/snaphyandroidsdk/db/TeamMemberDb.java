...
public class TeamMemberDb{
  ...
  public TeamMemberDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<TeamMember> getAll__lazy(final int limit, final int skip, ObjectCallback<TeamMember> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final TeamMemberRepository repo = restAdapter.createRepository(TeamMemberRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<TeamMember>() {
            @Override
            public TeamMember create(Cursor cursor, int index) {
                TeamMember data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (TeamMember)repo.createObject(hashMap);
                return data;
            }
    ...
    public TeamMember get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("TeamMember", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
