...
public class HomeWorkDb{
  ...
  public HomeWorkDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public void insert__db (final String id, final HomeWork modelData) {
    ...
    public ContentValues getContentValues(HomeWork modelData){
      ContentValues values = new ContentValues();
                        ...
                        if(modelData.getAttachment() != null){
                          attachmentData = new Gson().toJson(modelData.getAttachment(), HashMap.class);
                        }
                                                values.put("`attachment`", attachmentData);
    ...
    // Getting single c
    public   HomeWork get__db(String id) {
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("HomeWork", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    ...
                    //db.close(); // Closing database connection
                    if (hashMap != null) {
                        HomeWorkRepository repo = restAdapter.createRepository(HomeWorkRepository.class);
                        repo.addStorage(context);
                        return (HomeWork)repo.createObject(hashMap);
                    } else {
                        return null;
                    }
                }
    ...
    // Getting single cont
    public   HomeWork get__db(String whereKey, String whereKeyValue) {
        if (whereKeyValue != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("HomeWork", null, "`" + whereKey + "` =?", new String[]{whereKeyValue}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    return null;
                }else{
    ...
    private HashMap<String, Object> parseCursor(Cursor cursor ){
      HashMap<String, Object> hashMap = new HashMap<>();
                        ...
                        if(cursor.getString(4) != null){
                          attachmentData = new Gson().fromJson(cursor.getString(4), Map.class);
                          if(attachmentData != null){
                            attachmentData = (Map<String, Object>)attachmentData;
                            hashMap.put("attachment", attachmentData);
                          }
                        }
    ...
    public void upsert__db(String id, HomeWork model){
        if(count__db(id) != 0){
            update__db(id, model);
        }else{
            insert__db(id, model);
    ...
    public DataList<HomeWork>  getAll__db() {
        DataList<HomeWork> modelList = new DataList<HomeWork>();
        // Select All Query
        ...
        if (!cursor.moveToFirst() || cursor.getCount() == 0){
            return (DataList<HomeWork>) modelList;
        }else{
            do {
                ...
                if(hashMap != null){
                    HomeWorkRepository repo = restAdapter.createRepository(HomeWorkRepository.class);
                    repo.addStorage(context);
                    modelList.add((HomeWork)repo.createObject(hashMap));
                }
    ...
    public DataList<HomeWork>  getAll__db(HashMap<String, Object> whereKeyValue) {
    ...
    public DataList<HomeWork>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit) {
        DataList<HomeWork> modelList = new DataList<HomeWork>();
        String whereQuery = getWhereQuery(whereKeyValue);
        String selectQuery;
        if(orderBy != null){
            selectQuery = "SELECT  * FROM HomeWork " + whereQuery  + " ORDER BY " + orderBy ;
            if(limit != 0){
                // Select All Query
            ...
            if(limit != 0){
                // Select All Query
                selectQuery = "SELECT  * FROM HomeWork " + whereQuery + " LIMIT " + limit;
            }else{
                selectQuery = "SELECT  * FROM HomeWork " + whereQuery;
            }
        }
    ...
    // Getting All Data where
    public DataList<HomeWork>  getAll__db(HashMap<String, Object> whereKeyValue, int limit) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit){
        ...
        String countQuery;
        if(orderBy != null){
            countQuery = "SELECT  * FROM HomeWork " + whereQuery  + " ORDER BY " + orderBy ;
            if(limit != 0){
                // Select All Query
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM HomeWork " + whereQuery + " LIMIT " + limit;
            }else{
                countQuery = "SELECT  * FROM HomeWork " + whereQuery;
            }
        }
    ...
    public void checkOldData__db(final HashMap<String, Object> whereKeyValue) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                // updating row
                db.update("HomeWork", values, "_DATA_UPDATED = 1 AND " + where, null);
                db.setTransactionSuccessful();
                db.endTransaction();
                //db.close();
            }
    ...
    public void deleteOldData__db(final HashMap<String, Object> whereKeyValue) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                db.beginTransaction();
                String where = getWhere(whereKeyValue);
                db.delete("HomeWork", "_DATA_UPDATED = 0 AND " + where , null);
                db.setTransactionSuccessful();
                db.endTransaction();
                //db.close();
            }
    ...
    // Getting All Data where
    public DataList<HomeWork>  getAll__db(String whereKey, String whereKeyValue) {
        DataList<HomeWork> modelList = new DataList<HomeWork>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `HomeWork` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
    ...
    public int count__db(String whereKey, String whereKeyValue){
        String countQuery = "SELECT  * FROM `HomeWork` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
        Cursor cursor = db.rawQuery(countQuery, null);
        int count = cursor.getCount();
        cursor.close();
    ...
    public void checkOldData__db(final String whereKey, final String whereKeyValue) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                values.put("_DATA_UPDATED", 0);
                // updating row
                db.update("HomeWork", values, "_DATA_UPDATED = 1 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
                db.setTransactionSuccessful();
                db.endTransaction();
                //db.close();
            }
    ...
    public void deleteOldData__db(final String whereKey, final String whereKeyValue) {
        new Thread(new Runnable() {
            ...
            public void run() {
                SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
                db.beginTransaction();
                db.delete("HomeWork", "_DATA_UPDATED = 0 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
                db.setTransactionSuccessful();
                db.endTransaction();
                //db.close();
            }
    ...
    public void update__db(final String id,   final HomeWork modelData) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
    ...
    public void checkOldData__db() {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                values.put("_DATA_UPDATED", 0);
                // updating row
                db.update("HomeWork", values, "_DATA_UPDATED = 1", null);
                db.setTransactionSuccessful();
                db.endTransaction();
                //db.close();
            }
    ...
    public void deleteOldData__db() {
      new Thread(new Runnable() {
            ...
            public void run() {
                SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
                db.beginTransaction();
                db.delete("HomeWork", "_DATA_UPDATED = 0", null);
                db.setTransactionSuccessful();
                db.endTransaction();
                //db.close();
            }
