...
public class HospitalDb{
  ...
  public HospitalDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<Hospital> getAll__lazy(final int limit, final int skip, ObjectCallback<Hospital> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final HospitalRepository repo = restAdapter.createRepository(HospitalRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Hospital>() {
            @Override
            public Hospital create(Cursor cursor, int index) {
                Hospital data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Hospital)repo.createObject(hashMap);
                return data;
            }
    ...
    public Hospital get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Hospital", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
