...
public class NotificationDb{
  ...
  public NotificationDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<Notification> getAll__lazy(final int limit, final int skip, ObjectCallback<Notification> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final NotificationRepository repo = restAdapter.createRepository(NotificationRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Notification>() {
            @Override
            public Notification create(Cursor cursor, int index) {
                Notification data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Notification)repo.createObject(hashMap);
                return data;
            }
    ...
    public Notification get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Notification", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
