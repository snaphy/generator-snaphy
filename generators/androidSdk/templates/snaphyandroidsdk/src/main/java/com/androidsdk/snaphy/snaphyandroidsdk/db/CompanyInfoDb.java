...
public class CompanyInfoDb{
  ...
  public CompanyInfoDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<CompanyInfo> getAll__lazy(final int limit, final int skip, ObjectCallback<CompanyInfo> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final CompanyInfoRepository repo = restAdapter.createRepository(CompanyInfoRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<CompanyInfo>() {
            @Override
            public CompanyInfo create(Cursor cursor, int index) {
                CompanyInfo data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (CompanyInfo)repo.createObject(hashMap);
                return data;
            }
    ...
    public CompanyInfo get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("CompanyInfo", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
