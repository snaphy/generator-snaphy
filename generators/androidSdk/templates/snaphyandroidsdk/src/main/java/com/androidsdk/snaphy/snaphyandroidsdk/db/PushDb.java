...
public class PushDb{
  ...
  public PushDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<Push> getAll__lazy(final int limit, final int skip, ObjectCallback<Push> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final PushRepository repo = restAdapter.createRepository(PushRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Push>() {
            @Override
            public Push create(Cursor cursor, int index) {
                Push data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Push)repo.createObject(hashMap);
                return data;
            }
    ...
    public Push get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Push", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
