...
public class SnaphyAclPropDb{
  ...
  public SnaphyAclPropDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
    ...
    private HashMap<String, Object> parseCursor(Cursor cursor ){
      HashMap<String, Object> hashMap = new HashMap<>();
                        ...
                        if(cursor.getString(4) != null){
                          snaphyAclIdData = cursor.getString(4);
     ...
     * @return
     */
    public LazyList<SnaphyAclProp> getAll__lazy(final int limit, final int skip, ObjectCallback<SnaphyAclProp> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final SnaphyAclPropRepository repo = restAdapter.createRepository(SnaphyAclPropRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<SnaphyAclProp>() {
            @Override
            public SnaphyAclProp create(Cursor cursor, int index) {
                SnaphyAclProp data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (SnaphyAclProp)repo.createObject(hashMap);
                return data;
            }
    ...
    public SnaphyAclProp get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("SnaphyAclProp", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
