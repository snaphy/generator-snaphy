...
public class RoleDb{
  ...
  public RoleDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<Role> getAll__lazy(final int limit, final int skip, ObjectCallback<Role> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final RoleRepository repo = restAdapter.createRepository(RoleRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Role>() {
            @Override
            public Role create(Cursor cursor, int index) {
                Role data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Role)repo.createObject(hashMap);
                return data;
            }
    ...
    public Role get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Role", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
