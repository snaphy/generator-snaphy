...
public class InstallationDb{
  ...
  public InstallationDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<Installation> getAll__lazy(final int limit, final int skip, ObjectCallback<Installation> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final InstallationRepository repo = restAdapter.createRepository(InstallationRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Installation>() {
            @Override
            public Installation create(Cursor cursor, int index) {
                Installation data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Installation)repo.createObject(hashMap);
                return data;
            }
    ...
    public Installation get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Installation", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
