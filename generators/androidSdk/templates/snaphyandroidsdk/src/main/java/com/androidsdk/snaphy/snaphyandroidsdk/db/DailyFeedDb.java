...
public class DailyFeedDb{
  ...
  public DailyFeedDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public DataList<DailyFeed>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit) {
        DataList<DailyFeed> modelList = new DataList<DailyFeed>();
        String whereQuery = getWhereQuery(whereKeyValue);
        String selectQuery;
        if(orderBy != null){
            selectQuery = "SELECT  * FROM `DailyFeed` " + whereQuery  + " ORDER BY " + orderBy ;
            if(limit != 0){
                // Select All Query
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit){
        ...
        String countQuery;
        if(orderBy != null){
            countQuery = "SELECT  * FROM `DailyFeed` " + whereQuery  + " ORDER BY " + orderBy ;
            if(limit != 0){
                // Select All Query
    ...
    public void delete__db(final HashMap<String, Object> whereKeyValue) {
      new Thread(new Runnable() {
            ...
            public void run() {
                ...
                db.beginTransaction();
                String where = getWhere(whereKeyValue);
                db.delete("`DailyFeed`", where , null);
                db.setTransactionSuccessful();
                db.endTransaction();
            }
        }).start();
    ...
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final DailyFeed modelData ){
      new Thread(new Runnable(){
        @Override
        public void run(){
          SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
          ...
          ContentValues values = getContentValues(modelData);
          String where = getWhere(whereKeyValue);
          db.update("`DailyFeed`", values, where, null);
          db.setTransactionSuccessful();
          db.endTransaction();
          //db.close();
        }
