...
public class StateDb{
  ...
  public StateDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
    ...
    public void insert__db (final String id, final State _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      // Inserting Row
    ...
    public ContentValues getContentValues(State _modelData){
    ...
    // Getting single c
    public   State get__db(String id) {
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("State", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    ...
                    if (hashMap != null) {
                        StateRepository repo = restAdapter.createRepository(StateRepository.class);
                        repo.addStorage(context);
                        return (State)repo.createObject(hashMap);
                    } else {
                        return null;
                    }
                }
    ...
    // Getting single cont
    public   State get__db(String whereKey, String whereKeyValue) {
        if (whereKeyValue != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("`State`", null, "`" + whereKey + "` =?", new String[]{whereKeyValue}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    return null;
                }else{
    ...
    public void upsert__db(String id, State model){
        if(count__db(id) != 0){
            update__db(id, model);
        }else{
            insert__db(id, model);
    ...
    public DataList<State>  getAll__db() {
        ...
        if (!cursor.moveToFirst() || cursor.getCount() == 0){
            return (DataList<State>) modelList;
        }else{
            do {
                ...
                if(hashMap != null){
                    StateRepository repo = restAdapter.createRepository(StateRepository.class);
                    repo.addStorage(context);
                    modelList.add((State)repo.createObject(hashMap));
                }
     ...
     * @return
     */
    public LazyList<State> getAll__lazy(final int limit, final int skip, ObjectCallback<State> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final StateRepository repo = restAdapter.createRepository(StateRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<State>() {
            @Override
            public State create(Cursor cursor, int index) {
                State data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (State)repo.createObject(hashMap);
                return data;
            }
            ...
            public String getUpdateQuery() {
                ...
                if(orderBy != null){
                    ...
                    if(limit != 0){
                        ...
                        selectQuery = "SELECT  * FROM State " + whereQuery  + " LIMIT -1 OFFSET " + skip;
                    }
                }
    ...
    public State get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("State", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                ...
                countQuery = "SELECT  * FROM `State` " + whereQuery + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    public void checkOldData__db(final HashMap<String, Object> whereKeyValue) {
            ...
            // updating row
            db.update("`State`", values, "_DATA_UPDATED = 1 AND " + where, null);
    }
    ...
    public void deleteOldData__db(final HashMap<String, Object> whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      String where = getWhere(whereKeyValue);
      db.delete("`State`", "_DATA_UPDATED = 0 AND " + where , null);
    }
    ...
    // Getting All Data where
    public DataList<State>  getAll__db(String whereKey, String whereKeyValue) {
        DataList<State> modelList = new DataList<State>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `State` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
    ...
    public int count__db(String whereKey, String whereKeyValue){
      String countQuery = "SELECT  * FROM `State` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
      Cursor cursor = db.rawQuery(countQuery, null);
      int count = cursor.getCount();
      cursor.close();
    ...
    public void checkOldData__db(final String whereKey, final String whereKeyValue) {
      ...
      values.put("_DATA_UPDATED", 0);
      // updating row
      db.update("`State`", values, "_DATA_UPDATED = 1 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
    }
    ...
    public void deleteOldData__db(final String whereKey, final String whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      db.delete("`State`", "_DATA_UPDATED = 0 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
    }


    //Update multiple data at once..
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final State _modelData ){
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      String where = getWhere(whereKeyValue);
      db.update("`State`", values, where, null);
    }
    ...
    public void update__db(final String id,   final State _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`State`", values, "id = ?", new String[] { id });
    }
    ...
    public void checkOldData__db() {
      ...
      values.put("_DATA_UPDATED", 0);
      // updating row
      db.update("`State`", values, "_DATA_UPDATED = 1", null);
    }
    ...
    public void deleteOldData__db() {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      db.delete("`State`", "_DATA_UPDATED = 0", null);
    }
