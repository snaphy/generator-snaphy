...
public class BrandDb{
  ...
  public BrandDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
      ...
      ContentValues values = new ContentValues();
                        ...
                        if(_modelData.getName() != null){
                          ...
                          values.put("`name`", nameData);
                                                            ...
                                                            String websiteLinkData = "";
                        if(_modelData.getWebsiteLink() != null){
                          websiteLinkData = _modelData.getWebsiteLink().toString();
                          values.put("`websiteLink`", websiteLinkData);
                        }
    ...
    private HashMap<String, Object> parseCursor(Cursor cursor ){
      HashMap<String, Object> hashMap = new HashMap<>();
                        ...
                        if(cursor.getString(4) != null){
                          websiteLinkData = cursor.getString(4);
                          if(websiteLinkData != null){
                            websiteLinkData = (String)websiteLinkData;
                            hashMap.put("websiteLink", websiteLinkData);
                          }
                        }
     ...
     * @return
     */
    public LazyList<Brand> getAll__lazy(final int limit, final int skip, ObjectCallback<Brand> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final BrandRepository repo = restAdapter.createRepository(BrandRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Brand>() {
            @Override
            public Brand create(Cursor cursor, int index) {
                Brand data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Brand)repo.createObject(hashMap);
                return data;
            }
    ...
    public Brand get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Brand", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
