  ...
  public BrandDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public ContentValues getContentValues(Brand modelData){
      ContentValues values = new ContentValues();
                    ...
                    String brandVerificationIdData = "";
                    try {
                        Method method = modelData.getClass().getMethod("getBrandVerificationId");
                        if(method.invoke(modelData) != null){
                          //brandVerificationIdData = modelData.getBrandVerificationId().toString();
                          brandVerificationIdData = (String) method.invoke(modelData);
                          values.put("`brandVerificationId`", brandVerificationIdData);
                        }
                    } catch (Exception e) {
                      Log.e("Database Error", e.toString());
                    }
    ...
    private HashMap<String, Object> parseCursor(Cursor cursor ){
      HashMap<String, Object> hashMap = new HashMap<>();
                  ...
                  //End for loop
                    ...
                    String brandVerificationIdData = "";
                    int brandVerificationIdindex = cursor.getColumnIndex("brandVerificationId");
                    if(brandVerificationIdindex >= 0){

                        if(cursor.getString(brandVerificationIdindex) != null){
                          brandVerificationIdData = cursor.getString(brandVerificationIdindex);
                          if(brandVerificationIdData != null){
                            brandVerificationIdData = brandVerificationIdData.toString();
                            hashMap.put("brandVerificationId", brandVerificationIdData);
                          }
                        }
    ...
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final Brand modelData ){
      new Thread(new Runnable(){
        @Override
        public void run(){
          SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
