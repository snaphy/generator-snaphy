...
public class CompanyDb{
  ...
  public CompanyDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
    ...
    public void insert__db (final String id, final Company _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      // Inserting Row
    ...
    public ContentValues getContentValues(Company _modelData){
    ...
    // Getting single c
    public   Company get__db(String id) {
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Company", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    ...
                    if (hashMap != null) {
                        CompanyRepository repo = restAdapter.createRepository(CompanyRepository.class);
                        repo.addStorage(context);
                        return (Company)repo.createObject(hashMap);
                    } else {
                        return null;
                    }
                }
    ...
    // Getting single cont
    public   Company get__db(String whereKey, String whereKeyValue) {
        if (whereKeyValue != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("`Company`", null, "`" + whereKey + "` =?", new String[]{whereKeyValue}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    return null;
                }else{
    ...
    private HashMap<String, Object> parseCursor(Cursor cursor ){
      HashMap<String, Object> hashMap = new HashMap<>();
                        ...
                        if(cursor.getString(2) != null){
                          addressData = cursor.getString(2);
                          if(addressData != null){
    ...
    public void upsert__db(String id, Company model){
        if(count__db(id) != 0){
            update__db(id, model);
        }else{
            insert__db(id, model);
    ...
    public DataList<Company>  getAll__db() {
        DataList<Company> modelList = new DataList<Company>();
        // Select All Query
        ...
        if (!cursor.moveToFirst() || cursor.getCount() == 0){
            return (DataList<Company>) modelList;
        }else{
            do {
                ...
                if(hashMap != null){
                    CompanyRepository repo = restAdapter.createRepository(CompanyRepository.class);
                    repo.addStorage(context);
                    modelList.add((Company)repo.createObject(hashMap));
                }
     ...
     * @return
     */
    public LazyList<Company> getAll__lazy(final int limit, final int skip, ObjectCallback<Company> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final CompanyRepository repo = restAdapter.createRepository(CompanyRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Company>() {
            @Override
            public Company create(Cursor cursor, int index) {
                Company data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Company)repo.createObject(hashMap);
                return data;
            }
            ...
            public String getUpdateQuery() {
                ...
                String selectQuery;
                if(orderBy != null){
                    selectQuery = "SELECT  * FROM `Company` " + whereQuery  + " ORDER BY " + orderBy ;
                    if(limit != 0){
                        // Select All Query
    ...
    public Company get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Company", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
    ...
    public DataList<Company>  getAll__db(HashMap<String, Object> whereKeyValue) {
    ...
    // Getting All Data where and sort column according to date wise..
    public DataList<Company>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<Company> modelList = new DataList<Company>();
    ...
    // Getting All Data where
    public DataList<Company>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        String countQuery;
        if(orderBy != null){
            countQuery = "SELECT  * FROM `Company` " + whereQuery  + " ORDER BY " + orderBy ;
            if(limit != 0){
                // Select All Query
    ...
    public void checkOldData__db(final HashMap<String, Object> whereKeyValue) {
            ...
            // updating row
            db.update("`Company`", values, "_DATA_UPDATED = 1 AND " + where, null);
    }
    ...
    public void deleteOldData__db(final HashMap<String, Object> whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      String where = getWhere(whereKeyValue);
      db.delete("`Company`", "_DATA_UPDATED = 0 AND " + where , null);
    }
    ...
    // Getting All Data where
    public DataList<Company>  getAll__db(String whereKey, String whereKeyValue) {
        DataList<Company> modelList = new DataList<Company>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `Company` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
    ...
    public int count__db(String whereKey, String whereKeyValue){
      String countQuery = "SELECT  * FROM `Company` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
      Cursor cursor = db.rawQuery(countQuery, null);
      int count = cursor.getCount();
      cursor.close();
    ...
    public void checkOldData__db(final String whereKey, final String whereKeyValue) {
      ...
      values.put("_DATA_UPDATED", 0);
      // updating row
      db.update("`Company`", values, "_DATA_UPDATED = 1 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
    }
    ...
    public void deleteOldData__db(final String whereKey, final String whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      db.delete("`Company`", "_DATA_UPDATED = 0 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
    }


    //Update multiple data at once..
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final Company _modelData ){
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      String where = getWhere(whereKeyValue);
      db.update("`Company`", values, where, null);
    }
    ...
    public void update__db(final String id,   final Company _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`Company`", values, "id = ?", new String[] { id });
    }
    ...
    public void checkOldData__db() {
      ...
      values.put("_DATA_UPDATED", 0);
      // updating row
      db.update("`Company`", values, "_DATA_UPDATED = 1", null);
    }
    ...
    public void deleteOldData__db() {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      db.delete("`Company`", "_DATA_UPDATED = 0", null);
    }
