    ...
    public DataList<RegistrationCouncil>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                ...
                selectQuery = "SELECT  * FROM RegistrationCouncil " + whereQuery  + " OFFSET " + skip;
            }
        }
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        if(orderBy != null){
            ...
            if(limit != 0){
                ...
                countQuery = "SELECT  * FROM `RegistrationCouncil` " + whereQuery + " OFFSET " + skip;
            }
        }
    ...
    public void update__db(final String id,   final RegistrationCouncil _modelData) {
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                ContentValues values = getContentValues(_modelData);
                // updating row
                db.update("`RegistrationCouncil`", values, "id = ?",
                        new String[] { id });
                db.setTransactionSuccessful();
                db.endTransaction();
                //db.close();
