...
import android.content.Context;
import com.androidsdk.snaphy.snaphyandroidsdk.db.WishlistDb;
...
public class WishlistRepository extends ModelRepository<Wishlist> {
    ...
    public WishlistDb getDb() {
      return wishlistDb;
    }

    public void setWishlistDb(WishlistDb wishlistDb) {
    ...
    public void addStorage(Context context){
         ...
         setWishlistDb(new WishlistDb(context, DATABASE_NAME, getRestAdapter()));
    ...
    contract.addItem(new RestContractItem("/" + getNameForRestUrl() + "/:wishlistId/product", "GET"), "Wishlist.prototype.__get__product");
    ...
    contract.addItem(new RestContractItem("/" + getNameForRestUrl() + "/getDetailSchema", "POST"), "Wishlist.getDetailSchema");
    ...
    contract.addItem(new RestContractItem("/" + getNameForRestUrl() + "/getModelRelationSchema", "POST"), "Wishlist.getModelRelationSchema");
...
//override getNameForRestUrlMethod
    public String  getNameForRestUrl() {
            ...
            public void get__product(  String wishlistId,  Boolean refresh, final ObjectCallback<Product> callback){
                        ...
                        public void onSuccess(JSONObject response) {
                          try{
                                ...
                                if(response != null){
                                    ...
                                    if(context != null){
                                        try {
                                            Method method = wishlistRepo.getClass().getMethod("addStorage", Context.class);
                                            method.invoke(wishlistRepo, context);

                                        } catch (Exception e) {
                                            Log.e("Database Error", e.toString());
                                        }

                                        //wishlistRepo.addStorage(context);
                                    }
                                    Map<String, Object> result = Util.fromJson(response);
                                    Wishlist wishlist = wishlistRepo.createObject(result);
                                      ...
                                      if(isSTORE_LOCALLY()){
                                          //http://stackoverflow.com/questions/160970/how-do-i-invoke-a-java-method-when-given-the-method-name-as-a-string
                                          try {
                                                    Method method = wishlist.getClass().getMethod("save__db");
                                                    method.invoke(wishlist);
