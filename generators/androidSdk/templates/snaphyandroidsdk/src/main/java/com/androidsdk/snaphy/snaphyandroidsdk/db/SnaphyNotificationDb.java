...
public class SnaphyNotificationDb{
  ...
  public SnaphyNotificationDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<SnaphyNotification> getAll__lazy(final int limit, final int skip, ObjectCallback<SnaphyNotification> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final SnaphyNotificationRepository repo = restAdapter.createRepository(SnaphyNotificationRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<SnaphyNotification>() {
            @Override
            public SnaphyNotification create(Cursor cursor, int index) {
                SnaphyNotification data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (SnaphyNotification)repo.createObject(hashMap);
                return data;
            }
    ...
    public SnaphyNotification get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("SnaphyNotification", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
