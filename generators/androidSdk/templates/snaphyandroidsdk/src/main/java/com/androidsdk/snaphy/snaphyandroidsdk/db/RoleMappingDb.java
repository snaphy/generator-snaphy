...
public class RoleMappingDb{
  ...
  public RoleMappingDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<RoleMapping> getAll__lazy(final int limit, final int skip, ObjectCallback<RoleMapping> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final RoleMappingRepository repo = restAdapter.createRepository(RoleMappingRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<RoleMapping>() {
            @Override
            public RoleMapping create(Cursor cursor, int index) {
                RoleMapping data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (RoleMapping)repo.createObject(hashMap);
                return data;
            }
    ...
    public RoleMapping get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("RoleMapping", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
