...
public class BillDb{
  ...
  public BillDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<Bill> getAll__lazy(final int limit, final int skip, ObjectCallback<Bill> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final BillRepository repo = restAdapter.createRepository(BillRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Bill>() {
            @Override
            public Bill create(Cursor cursor, int index) {
                Bill data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Bill)repo.createObject(hashMap);
                return data;
            }
    ...
    public Bill get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Bill", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
