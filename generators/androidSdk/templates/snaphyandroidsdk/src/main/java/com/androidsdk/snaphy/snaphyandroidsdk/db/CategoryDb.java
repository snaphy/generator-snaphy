...
public class CategoryDb{
  ...
  public CategoryDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
    ...
    public void insert__db (final String id, final Category _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      // Inserting Row
     ...
     * @return
     */
    public LazyList<Category> getAll__lazy(final int limit, final int skip, ObjectCallback<Category> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final CategoryRepository repo = restAdapter.createRepository(CategoryRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Category>() {
            @Override
            public Category create(Cursor cursor, int index) {
                Category data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Category)repo.createObject(hashMap);
                return data;
            }
            ...
            public String getUpdateQuery() {
                ...
                String selectQuery;
                if(orderBy != null){
                    selectQuery = "SELECT  * FROM `Category` " + whereQuery  + " ORDER BY " + orderBy ;
                    if(limit != 0){
                        // Select All Query
                    ...
                    if(limit != 0){
                        // Select All Query
                        selectQuery = "SELECT  * FROM Category " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
                    }else{
                        selectQuery = "SELECT  * FROM Category " + whereQuery  + " LIMIT -1 OFFSET " + skip;
                    }
                }
    ...
    public Category get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("Category", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
    ...
    public DataList<Category>  getAll__db(HashMap<String, Object> whereKeyValue) {
    ...
    // Getting All Data where and sort column according to date wise..
    public DataList<Category>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<Category> modelList = new DataList<Category>();
    ...
    // Getting All Data where
    public DataList<Category>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        String countQuery;
        if(orderBy != null){
            countQuery = "SELECT  * FROM `Category` " + whereQuery  + " ORDER BY " + orderBy ;
            if(limit != 0){
                // Select All Query
            ...
            if(limit != 0){
                // Select All Query
                countQuery = "SELECT  * FROM `Category` " + whereQuery + " LIMIT " + limit + " OFFSET " + skip;
            }else{
                countQuery = "SELECT  * FROM `Category` " + whereQuery + " LIMIT -1 OFFSET " + skip;
            }
        }
    ...
    public void checkOldData__db(final HashMap<String, Object> whereKeyValue) {
            ...
            // updating row
            db.update("`Category`", values, "_DATA_UPDATED = 1 AND " + where, null);
    }
    ...
    public void deleteOldData__db(final HashMap<String, Object> whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      String where = getWhere(whereKeyValue);
      db.delete("`Category`", "_DATA_UPDATED = 0 AND " + where , null);
    }
    ...
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final Category _modelData ){
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      String where = getWhere(whereKeyValue);
      db.update("`Category`", values, where, null);
    }
    ...
    public void update__db(final String id,   final Category _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`Category`", values, "id = ?", new String[] { id });
    }
