...
public class MergeRequestDb{
  ...
  public MergeRequestDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<MergeRequest> getAll__lazy(final int limit, final int skip, ObjectCallback<MergeRequest> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final MergeRequestRepository repo = restAdapter.createRepository(MergeRequestRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<MergeRequest>() {
            @Override
            public MergeRequest create(Cursor cursor, int index) {
                MergeRequest data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (MergeRequest)repo.createObject(hashMap);
                return data;
            }
    ...
    public MergeRequest get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("MergeRequest", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
