...
public class FeedDb{
  ...
  public FeedDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<Feed> getAll__lazy(final int limit, final int skip, ObjectCallback<Feed> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final FeedRepository repo = restAdapter.createRepository(FeedRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<Feed>() {
            @Override
            public Feed create(Cursor cursor, int index) {
                Feed data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (Feed)repo.createObject(hashMap);
                return data;
            }
    ...
    public void update__db(final String id,   final Feed _modelData) {
      ...
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`Feed`", values, "id = ?", new String[] { id });
    }
