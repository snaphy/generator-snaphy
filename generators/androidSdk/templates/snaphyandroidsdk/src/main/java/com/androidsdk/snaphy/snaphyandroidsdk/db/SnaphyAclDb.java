...
public class SnaphyAclDb{
  ...
  public SnaphyAclDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
     ...
     * @return
     */
    public LazyList<SnaphyAcl> getAll__lazy(final int limit, final int skip, ObjectCallback<SnaphyAcl> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final SnaphyAclRepository repo = restAdapter.createRepository(SnaphyAclRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<SnaphyAcl>() {
            @Override
            public SnaphyAcl create(Cursor cursor, int index) {
                SnaphyAcl data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (SnaphyAcl)repo.createObject(hashMap);
                return data;
            }
    ...
    public SnaphyAcl get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("SnaphyAcl", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
