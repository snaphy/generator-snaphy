...
public class UserInviteDb{
  ...
  public UserInviteDb(Context context, String DATABASE_NAME, SnaphyRestAdapter restAdapter){
    ...
    public void insert__db (final String id, final UserInvite _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      // Inserting Row
      ContentValues values = getContentValues(_modelData);
      db.insert("`UserInvite`", null, values);
    }
    ...
    public ContentValues getContentValues(UserInvite _modelData){
    ...
    // Getting single c
    public   UserInvite get__db(String id) {
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("UserInvite", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    ...
                    if (hashMap != null) {
                        UserInviteRepository repo = restAdapter.createRepository(UserInviteRepository.class);
                        repo.addStorage(context);
                        return (UserInvite)repo.createObject(hashMap);
                    } else {
                        return null;
                    }
                }
    ...
    // Getting single cont
    public   UserInvite get__db(String whereKey, String whereKeyValue) {
        if (whereKeyValue != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("`UserInvite`", null, "`" + whereKey + "` =?", new String[]{whereKeyValue}, null, null, null, null);
            if (cursor != null) {
                if (!cursor.moveToFirst() || cursor.getCount() == 0){
                    return null;
                }else{
    ...
    private HashMap<String, Object> parseCursor(Cursor cursor ){
      HashMap<String, Object> hashMap = new HashMap<>();
                        ...
                        if(cursor.getString(1) != null){
                          expiryDateData = cursor.getString(1);
    ...
    public void upsert__db(String id, UserInvite model){
        if(count__db(id) != 0){
            update__db(id, model);
        }else{
            insert__db(id, model);
    ...
    public DataList<UserInvite>  getAll__db() {
        DataList<UserInvite> modelList = new DataList<UserInvite>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `UserInvite`";

        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
        //http://www.tothenew.com/blog/sqlite-locking-and-transaction-handling-in-android/
        Cursor cursor = db.rawQuery(selectQuery, null);
        if (!cursor.moveToFirst() || cursor.getCount() == 0){
            return (DataList<UserInvite>) modelList;
        }else{
            do {
                ...
                if(hashMap != null){
                    UserInviteRepository repo = restAdapter.createRepository(UserInviteRepository.class);
                    repo.addStorage(context);
                    modelList.add((UserInvite)repo.createObject(hashMap));
                }
     ...
     * @return
     */
    public LazyList<UserInvite> getAll__lazy(final int limit, final int skip, ObjectCallback<UserInvite> callback, final GetUpdatedQuery getUpdatedQuery, final OnParseCursor onParseCursor) {


        SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();

        final UserInviteRepository repo = restAdapter.createRepository(UserInviteRepository.class);
        repo.addStorage(context);
        return new LazyList<>(db, new LazyList.ItemFactory<UserInvite>() {
            @Override
            public UserInvite create(Cursor cursor, int index) {
                UserInvite data;
                cursor.moveToPosition(index);
                HashMap<String, Object> hashMap = onParseCursor.parseCursor(cursor);
                data = (UserInvite)repo.createObject(hashMap);
                return data;
            }
            ...
            public String getUpdateQuery() {
                ...
                String selectQuery;
                if(orderBy != null){
                    selectQuery = "SELECT  * FROM `UserInvite` " + whereQuery  + " ORDER BY " + orderBy ;
                    if(limit != 0){
                        // Select All Query
    ...
    public UserInvite get__db(String id, OnParseCursor onParseCursor){
        if (id != null) {
            SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
            Cursor cursor = db.query("UserInvite", null, "id=?", new String[]{id}, null, null, null, null);
            if (cursor != null) {
    ...
    public DataList<UserInvite>  getAll__db(HashMap<String, Object> whereKeyValue) {
    ...
    // Getting All Data where and sort column according to date wise..
    public DataList<UserInvite>  getAll__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip) {
        DataList<UserInvite> modelList = new DataList<UserInvite>();
    ...
    // Getting All Data where
    public DataList<UserInvite>  getAll__db(HashMap<String, Object> whereKeyValue, int limit, int skip) {
    ...
    public int count__db(HashMap<String, Object> whereKeyValue, String orderBy, int limit, int skip){
        ...
        String countQuery;
        if(orderBy != null){
            countQuery = "SELECT  * FROM `UserInvite` " + whereQuery  + " ORDER BY " + orderBy ;
            if(limit != 0){
                // Select All Query
    ...
    public void checkOldData__db(final HashMap<String, Object> whereKeyValue) {
            ...
            // updating row
            db.update("`UserInvite`", values, "_DATA_UPDATED = 1 AND " + where, null);
    }
    ...
    public void deleteOldData__db(final HashMap<String, Object> whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      String where = getWhere(whereKeyValue);
      db.delete("`UserInvite`", "_DATA_UPDATED = 0 AND " + where , null);
    }
    ...
    public void delete__db(final HashMap<String, Object> whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      String where = getWhere(whereKeyValue);
      db.delete("`UserInvite`", where , null);
    }
    ...
    // Getting All Data where
    public DataList<UserInvite>  getAll__db(String whereKey, String whereKeyValue) {
        DataList<UserInvite> modelList = new DataList<UserInvite>();
        // Select All Query
        String selectQuery = "SELECT  * FROM `UserInvite` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
    ...
    public int count__db(String whereKey, String whereKeyValue){
      String countQuery = "SELECT  * FROM `UserInvite` WHERE `" + whereKey +"` ='"+ whereKeyValue + "'" ;
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getReadableDatabase();
      Cursor cursor = db.rawQuery(countQuery, null);
      int count = cursor.getCount();
      cursor.close();
    ...
    public void checkOldData__db(final String whereKey, final String whereKeyValue) {
      ...
      values.put("_DATA_UPDATED", 0);
      // updating row
      db.update("`UserInvite`", values, "_DATA_UPDATED = 1 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
    }
    ...
    public void deleteOldData__db(final String whereKey, final String whereKeyValue) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      db.delete("`UserInvite`", "_DATA_UPDATED = 0 AND `" + whereKey + "` = ?", new String[]{whereKeyValue});
    }


    //Update multiple data at once..
    public void updateAll__db(final HashMap<String, Object> whereKeyValue, final UserInvite _modelData ){
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      String where = getWhere(whereKeyValue);
      db.update("`UserInvite`", values, where, null);
    }
    ...
    public void update__db(final String id,   final UserInvite _modelData) {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      ContentValues values = getContentValues(_modelData);
      // updating row
      db.update("`UserInvite`", values, "id = ?", new String[] { id });
    }
    ...
    public void checkOldData__db() {
      ...
      values.put("_DATA_UPDATED", 0);
      // updating row
      db.update("`UserInvite`", values, "_DATA_UPDATED = 1", null);
    }
    ...
    public void deleteOldData__db() {
      SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
      db.delete("`UserInvite`", "_DATA_UPDATED = 0", null);
    }
