...
public class ImportDataDb{
  ...
  public ImportDataDb(Context context, String DATABASE_NAME, RestAdapter restAdapter){
    ...
    public void insert__db (final String id, final ImportData modelData) {
    ...
    public ContentValues getContentValues(ImportData modelData){
    ...
    public void update__db(final String id,   final ImportData modelData) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                SQLiteDatabase db = DbHandler.getInstance(context, DATABASE_NAME).getWritableDatabase();
